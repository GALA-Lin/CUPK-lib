...
### 实验 1：固定 k=4，改变 Memory Size m

| m       | k    | FP 数 | 实际 FPR | 理论 FPR | 填充率 |
| ------- | ---- | ----- | -------- | -------- | ------ |
| 50,000  | 4    | 0     | 0.0000   | 0.0000   | 0.006  |
| 100,000 | 4    | 0     | 0.0000   | 0.0000   | 0.003  |
| 200,000 | 4    | 0     | 0.0000   | 0.0000   | 0.002  |
| 400,000 | 4    | 0     | 0.0000   | 0.0000   | 0.001  |

### 实验 2：固定 m=200,000，改变 hash 函数个数 k

| m       | k    | FP 数 | 实际 FPR | 理论 FPR | 填充率 |
| ------- | ---- | ----- | -------- | -------- | ------ |
| 200,000 | 1    | 7     | 0.0007   | 0.0004   | 0.000  |
| 200,000 | 2    | 0     | 0.0000   | 0.0000   | 0.001  |
| 200,000 | 4    | 0     | 0.0000   | 0.0000   | 0.002  |
| 200,000 | 6    | 0     | 0.0000   | 0.0000   | 0.002  |
| 200,000 | 8    | 0     | 0.0000   | 0.0000   | 0.003  |
| 200,000 | 10   | 0     | 0.0000   | 0.0000   | 0.004  |
| 200,000 | 12   | 0     | 0.0000   | 0.0000   | 0.004  |

### 实验3：综合对比 —— m×k网格（实际FPR）

| m       | k=2    | k=4    | k=6    | k=8    |
| ------- | ------ | ------ | ------ | ------ |
| 50,000  | 0.0000 | 0.0000 | 0.0000 | 0.0000 |
| 100,000 | 0.0000 | 0.0000 | 0.0000 | 0.0000 |
| 200,000 | 0.0000 | 0.0000 | 0.0000 | 0.0000 |

### 理论最优 hash 函数个数（k* = m/n × ln2）
...
插入时每个元素会将 k 个哈希位置置为 1。随着插入元素数量增加，bit array 会变得越来越密集。当查询一个未插入的元素时，如果它对应的 k 个哈希位置恰好全部被其他元素“顺带”置为 1，布隆过滤器会误判该元素存在，这就是 false positive 的来源。

### Q2. 为什么 Bloom Filter 不会出现 false negative？

布隆过滤器的插入操作只会将比特位置 1，**不会将任何位置 0**。一旦元素被插入，它对应的 k 个位置会永远保持为 1。查询时会检查同样的 k 个位置，因此已插入的元素一定全部命中，绝不会出现漏判，即 false negative 为 0。

### Q3. memory size 增大后误判率如何变化？

内存大小 m 增大 → bit array 更稀疏 → 各位被置 1 的概率降低 → 误判率 FPR 明显降低。
根据理论公式 `FPR = (1 - e^(-kn/m))^k`，m 越大，FPR 越趋近于 0。
实验 1 验证：m 从 50,000 增加到 400,000，填充率持续下降，FPR 保持为 0。
...
哈希函数个数 k 增大会产生两个相反效果：
① 每个元素占用更多位 → bit array 填充更快 → 冲突变多，FPR 趋向升高；
② 查询需要更多位同时为 1 → 误判概率趋向下降。
两者存在理论最优平衡点：`k* = (m/n) × ln2`。
实验 2 可见，k=1 时 FPR 最高，k≥2 后 FPR 降为 0，符合最优 k 的规律。
...
① 空间极省：只存储 bit array，不保存原始数据，空间复杂度 O(m)；
② 查询极快：每次只需 k 次哈希计算 + 位运算，时间复杂度 O(k)；
③ 零漏判：false negative rate = 0，“判定不存在则一定不存在”；
④ 可控误判：通过调整 m 和 k 可将 FPR 控制在业务可接受范围。
典型应用场景：数据库缓存穿透防护（防范恶意登陆等，规避数据库中不存在的账号）、分布式黑名单过滤等。