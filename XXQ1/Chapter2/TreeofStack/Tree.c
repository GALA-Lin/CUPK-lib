...
BROTHER ExpandNodes(TREE tree, NODE pa) //由节点获取子节点
{
    ...
    TREE t = tree; //树
    while (t) { //节点不为空
        if (EqualFun(t->node.node, pa) == 0) { //找到分支节点
...
STATUS SearchPath(TREE tree, NODE start, NODE end, PATH *path) //判断节点是否在树中, 并求取路径
{
    ...
    while (stack) //堆栈不为空继续搜索
    {
        ...
        if (EqualFun(end, node) == 0) //是否为所求目标节点
        {
            flag = TRUE; //修正标识
            *path = CopyPath(tempath); //获取路径
            break;
...
//(11)特定路径求解
void Path_Specific(TREE *tree, PATH *path)
{
...
int  main(int argc, char const *argv[]) {//测试求解路径c2
    ...
    char *filename = "F:\\VS_code_C\\xiaoxueqi\\Chapter2\\TreeofStack\\Tree.txt";  //树数据文件
    tree = CreateTree(tree, filename);

    printf("\n==================\n");
    Path_Specific(&tree, &path);
    RevPath(path);
    priPath(path);
    printf("\n==================\n\n");
